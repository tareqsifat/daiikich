                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col">
                            <p class="small text-muted mb-0">
                                <span class="fe fe-arrow-down fe-12"></span>
                                <span class="fs-14 text-light">{{ translate('Total Token Amount') }}</span>
                            </p>
                            <h3 class="mb-0 text-white fs-30">
                                @php
                                    $tokenDetails = \App\Models\AffiliateTokenTransfer::where('user_id', Auth::user()->id)->get();
                                    $total = 0;
                                    foreach ($tokenDetails as $key => $tokenDetail) {
                                        if ($tokenDetail->order != null && $tokenDetail->order->payment_status == 'paid') {
                                            $total += $tokenDetail->price;
                                        }
                                    }
                                @endphp
                                {{ single_price($total) }}
